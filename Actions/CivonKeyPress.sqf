...

{
	...
	if(INV_shortcuts)then{INV_shortcuts=false; titletext["@ILG keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["@ILG keys on", "PLAIN DOWN"];[] execVM "actions\civactions.sqf"};};
	//L key
	case 38: 
	...
	_pitted = _vcl getVariable "hasbeenpitted";
	if (isEngineOn _vcl && !isnil "_pitted") then
	{
		_fuel = fuel _vcl;
		if (_pitted==1) then 
		{
			_vcl setfuel (_fuel - 0.00215);
		};
	};
