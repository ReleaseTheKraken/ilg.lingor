...
while {true} do 
	...
	if (_MaxPos == -1) then 
		...
		if (_currentMayor == _MaxPos) then 
			...
			format["hint format[localize ""STRS_mayor_new"", ""%3"", %2]; if ((rolenumber-1) == %1) then {isMayor = true; true call TurnMayorFunc; ["LIN_ElPresidente"] call clothes; call ILG_ElPresidente;} else {isMayor = false;false call TurnMayorFunc;};", _MaxPos, _MaxStimmen, _MayorString] call broadcast;
			};		
