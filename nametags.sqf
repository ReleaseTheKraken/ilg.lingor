	...
	if(count _objs > 0 and INV_shortcuts)then
		...
		if (_tag != player and _tag in playerarray and alive _tag) then 
			...
			if(_tag in civarray)then
				...
				{
					_colorset = false;
					if (_tag in esuslots) then
					{
						_control2 ctrlSetText format ["ESU (%1)",(name _tag)];
						_control2 ctrlSetTextColor [1, 1, 0, 0.8];
						_colorset = true;
					};
					/*
					if (_tag in donatorlevel4) then
					{
						_control2 ctrlSetTextColor [1, 0.565, 0.314, 0.8];
						_colorset = true;
					};*/
					
					if (!_colorset) then
					{
						_control2 ctrlSetTextColor [1, 1, 1, 0.8];
					};
				};
