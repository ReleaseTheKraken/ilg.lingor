...

{
	...
	if(_Mishy == 0)exitwith{player groupchat localize "STRS_inv_buyitems_keinMishy"; shopactivescript = 0; _exitvar = 1};
	...
	if (_itemart == "Item" and instock) then 
		...
		if ((_gesamtgewicht + _inventargewicht) > INV_Tragfaehigkeit) then 
			...
			_cost  = _menge*_CostMitTax;
			if (_menge <= 0) exitWith {player groupChat localize "STRS_inv_buyitems_maxgewicht"; shopactivescript = 0; _exitvar = 1;};
		...
		if (_Mishy < _cost) then 
			...
			_cost  = _menge*_CostMitTax;																														
			if (_menge <= 0) exitWith {player groupChat localize "STRS_inv_buyitems_keinMishy"; shopactivescript = 0; _exitvar = 1;};
...

	{
		...
		if (!(_license1 call INV_HasLicense) and isciv and _license) exitWith {player groupChat format[localize "STRS_inv_buyitems_nolicense", (_license1 call INV_GetLicenseName)]; shopactivescript = 0; _exitvar = 1};
		if (!(_license2 call INV_HasLicense) and iscop and _license) exitWith {player groupChat format[localize "STRS_inv_buyitems_nolicensecop", (_license2 call INV_GetLicenseName)];  shopactivescript = 0; _exitvar = 1};
		if(_fahne == mafs && !("mafial" call INV_HasLicense)) exitwith {Player groupchat "You need mafia license for vehicles in this shop."};
