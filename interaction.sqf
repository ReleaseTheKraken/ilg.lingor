    ...
    [] spawn {  // You can't sleep or suspend something you call so use [] spawn in stead
        ...
        if (isciv) then {       
            ...
            if (Antworts == 1) then {
				...
				// Passes Variable _civ to ILG_999HandlerMP (Called on all remotes)
                format["[%1] call ILG_999HandlerMP", _civ] call broadcast;
            };
            if (Antworts == 2) then {
        ...
        _this spawn {  
            ...
            if (!(isNull(_civ))) then { 
                if (iscop) then {
                    ...
                    player sidechat format["%1, has dialed 911 to report an emergency! His location has been marked on the map. Dispatch units to his location ASAP!", _civ];
					player sidechat format["%1 has had his position marked as Civilian Called 911 from here! Check your maps and dispatch units ASAP.", _civ];
                };
            };
        };
    };
