...
_type   = typeof _car;
	...
	if ((player == driver _car) and (_car distance _x < 40) and (_speed >= 91) and !(_car iskindof "Air")) then 
		...
		if (_car iskindof "truck") then {_license = "truck"} else {_license = "car"};
		...
		(format['if(%3 == 0) then {server globalchat "%1 has lost his %2 license!";if(player == %1)then{INV_LizenzOwner = INV_LizenzOwner - ["%2"]; [player, [["license", INV_LizenzOwner]]] call LinLib_fnc_ClientUpdate;};}', player, _license, demerits]) call broadcast; 
